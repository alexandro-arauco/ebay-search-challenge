...
const ERROR_MESSAGES: Record<ApiErrorCode, { title: string; description: string }> = {
  ...
  EBAY_AUTH_FAILED: {
    ...
    description: "Could not connect to eBay. Check your API credentials and try again.",
  },
  EBAY_API_ERROR: {
    title: "eBay API error",
    description: "eBay returned an error. This may be a temporary issue — please try again.",
  },
  RATE_LIMITED: {
    title: "Rate limited",
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 gap-4 animate-fade-in">
      ...
      <div className="text-center">
        <h3 className="font-display text-lg font-semibold text-ink">{info.title}</h3>
        <p className="mt-1 text-sm text-ink-400 max-w-xs">{message ?? info.description}</p>
      </div>
      {onRetry && (
        <button
          onClick={onRetry}
          className="mt-2 px-5 py-2 bg-ink text-surface rounded-xl text-sm font-medium hover:bg-ink-700 transition-colors"
        >
          Try again
        </button>
      )}
    ...
    <div className="flex flex-col items-center justify-center py-20 gap-4 animate-fade-in">
      ...
      <div className="text-center">
        ...
        <p className="mt-1 text-sm text-ink-400 max-w-xs">
          No listings matched <span className="font-medium text-ink">&ldquo;{query}&rdquo;</span>. Try different keywords or adjust your filters.
        </p>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-col items-center justify-center py-24 gap-3 animate-fade-in">
      ...
      <div className="text-center mt-2">
        <h2 className="font-display text-xl font-semibold text-ink">Find anything on eBay</h2>
